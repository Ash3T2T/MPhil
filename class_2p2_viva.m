...
'20180918143525-m4dg' % #185 % F---- day one
'20180918144202-'
'20180918144315-stop'
'20180918150238-stop'
'20180919164949-stop'
...
% % %         M = 5; m = 1:M;       
        WW(:,1:5) = MMGs((idx+offset*(MULT-1):idx+offset*(MULT)),1:7); 
        M = 5; m = 1:M;
   elseif EXP == 0
% % %         WW(:,1:4) = ADCData2_BPfilt((idx+offset*(MULT-1):idx+offset*(MULT)),[1:3,5]); % ORIGINAL BEFORE PADDING APPLIED
% % %         M = 4; m = 1:M;       
        WW(:,1:4) = MMGs((idx+offset*(MULT-1):idx+offset*(MULT)),1:7); 
        M = 4; m = 1:M;
   end
   end
...
if A ==2 
D = nchoosek([1:6],activities)    % try this as 5 choose 5
