...
'alan_flex'
'alan_walk'
'alan_2legs_walk-rampx5'
'lan_dorsi-plantar'
'alan_ramp2x5-extratim'
'alan_plantrx10'
'alan_x10dorii-x10plamntar-nosocket'
'Alan_nosocketetstwithliner' % Leeds 2.2 10th Septmber onwards  :: currently one/two IMUs, 25 (nd 21) i believe
'Alan_nosocketetstwithliner_2' % #2 = #96
'Alanwlk5MMGsscket_1' % 97
'20180910142744-m4dg' % 10th Septemebr 2018, ALan day one, testing
'20180910143805-m4dg'
'20180910145034-m4dg' % 100
...
'20180911140907-m4dg' % 101 % 11th September Alan day one experimnt
...
...
'20180912114220-  save3  ' % 115 % Alan day two experiment 
'20180912114710-065'
'20180912114947-stop'
'20180912115312-stop'
'20180912122932-stop'
...
'20180914125632-stop' % me day one % #134
'20180914145957-save  '
'20180914151443-                              '
'20180914152657-m4dg'
'20180914154042-m4dg'
...
'20180915151812-stop' % me day two % #156
'20180915152425-stop'
'20180915153044-stop'
'20180915154157-stop'
'20180915155459-stop'
...
'20180918143525-m4dg' % #185 % Farid day one
'20180918144202-'
'20180918144315-stop'
'20180918150238-stop'
'20180919164949-stop'
...
%     idx = floor(heel_strike(rr)/1000); % should work for old and new Leeds2.0
    idx = heel_strike(rr); % should work for old and new Leeds2.0
...
elseif (win_scheme == 20 || win_scheme == 3) && ((idx+offset) <=  size(TIMData1,1)) && ((idx-offset) >= 1)
           offset = win; 
           mult = 1:2; MULT = 0:1;  
elseif (win_scheme == 21 || win_scheme == 3) && ((idx+offset*2) <=  size(TIMData1,1)) && ((idx-offset) < 1)
           offset = win; 
           mult = 1:2; MULT = 1:2;
elseif (win_scheme == 1 || win_scheme == 21 || win_scheme == 20 || win_scheme == 3) && ((idx+offset) <=  size(TIMData1,1)) && ((idx-offset) < 1)
           offset = win; 
           mult = 1; MULT = 1;
...
%    end
     ...
     feature_(m+(M*(wn-1)),rr,1) = sum(PKS_maxW > rangeW_)  + sum(-PKS_minW > rangeW_);           
     feature_(m+(M*(wn-1)),rr,2) = sum(-PKS_minW < rangeW_) +  sum(PKS_maxW < rangeW_);        
                                
     feature_(m+(M*(wn-1)),rr,3) = sum(abs(X-mean(X)));     % normalises       % IMMG 3.1.1

                feature_(m+(M*(wn-1)),rr,4) = sum(X);     % no normlaisation     % IMMG 3.1.1
                       
                feature_(m+(M*(wn-1)),rr,5) = mean(abs(X-mean(X)));    % normalisation    % MAV  3.1.2
                             
                feature_(m+(M*(wn-1)),rr,6) = mean(X);    % no mormalisation % MAV  3.1.2
                 
                feature_(m+(M*(wn-1)),rr,7) = sum(X.*X);   % simple square integral     % 3.1.6
                
                feature_(m+(M*(wn-1)),rr,8) = mean(X.*X);    % Variance (1/(N-1)) ?       % 3.1.7
                    
                feature_(m+(M*(wn-1)),rr,9) = sqrt(mean(X.*X));    % RMS    Root Mean Square    % 3.1.8
              ...
              % counting functions
                ...
                feature_(m+(M*(wn-1)),rr,10) = mean(sum_3);      % normalisation    % MMAV1 3.1.3
                                
                feature_(m+(M*(wn-1)),rr,11) = mean(sum_3b);     % no normalisation % MMAV1 3.1.3 
                
                feature_(m+(M*(wn-1)),rr,12) = mean(sum_4);      % normalisation     % MMAV2 3.1.4
                
                feature_(m+(M*(wn-1)),rr,13) = mean(sum_4b);     % no normalisation  % MMAV2 3.1.4
                
                feature_(m+(M*(wn-1)),rr,14) = sum(sum_9);       % Waveform length % 3.1.9
                 
                feature_(m+(M*(wn-1)),rr,15) = sum(sum_10);      % midline crossings  REMEMBER, WHEN FLTERED, USE ZERO CROSSINGS
                
                feature_(m+(M*(wn-1)),rr,16) = WAMP_count;       %% Willsion Amplitude    3.1.12
...
if A ==2 
D = nchoosek([1:6],6)    % try this as 5 choose 5
...
row = [1:train_acts];
test_acts = 2;
test_length = nchoosek(6,test_acts) % size(nchoosek(1:5,2),1) ; % corresponds to 5choose2 or ten tsts
D = [];
   for d = 1:test_length
       D = [D; row] ;
